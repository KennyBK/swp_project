...
public class ClassEvalDAO extends DBContext<UserAccount> {
    ...
    public ArrayList<Milestone> listMilestone(int uid) {
        
        ArrayList<Milestone> milestones = new ArrayList<>();
        try {
            String sql = "SELECT milestone_id, title FROM swp391.milestone where milestone_id in \n"
                    + "(select milestone_id from swp391.milestone where class_id in \n"
                    + "(select class_id from swp391.class where trainer_id =" + uid +"))";
    ...
    public ArrayList<EvalCriteria> getAllEvalCriteria(String status, String name, String assId) {
        ...
        try {
            String sql = "SELECT * FROM swp391.eval_criteria e, swp391.assignment a "
    ...
    public ArrayList<EvalCriteria> getAllEvalCriteriaByMilestone(String status, String name, String assId, String miId,String cid) {
        ...
        try {
            String sql = "SELECT * FROM swp391.eval_criteria e, swp391.assignment a, swp391.milestone m "
                    + "where " + statusCon + "  e.criteria_name like '%" + name + "%' and e.ass_id like '%" + assId + "%' "
                    + "and e.ass_id = a.ass_id and m.milestone_id = e.milestone_id and m.class_id like '%" + cid + "%'";
    ...
    public ArrayList<Assignment1> getAllAssignment() {
        ...
        try {
            String sql = "SELECT * FROM swp391.assignment;";
            PreparedStatement ps = connection.prepareStatement(sql);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public Assignment1 getAssignment(String aid) {
        try {
            String sql = "SELECT * FROM swp391.assignment a ,swp391.subject s \n"
    ...
    public static void main(String[] args) {
        ...
        System.out.println(a.getAllEvalCriteriaByMilestone("", "", "", "", "").size());
        System.out.println(a.getAllAssignment().size());
        System.out.println(a.listMilestone(2).size());
    }
}
